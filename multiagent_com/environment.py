...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                ...
                # given the power and trajectory control action space
                # 4*5 = 20
                u_action_space = spaces.Discrete( 20 )
                # u_action_space = spaces.Discrete(world.dim_p * 2 + 1)
            else:
                #u_action_space = spaces.Box(low=-agent.u_range, high=+agent.u_range, shape=(world.dim_p,), dtype=np.float32)
                u_action_space = spaces.Box(low=0, high=+agent.u_range, shape=(world.dim_p+world.dim_p_c,), dtype=np.float32)
            if agent.movable:
            ...
            #初始化power控制
            agent.action.p = np.zeros(self.world.dim_p_c)
    ...
    def trans_aciton(self, actions):
        return [self.dec_to_base(action, 10) for action in actions]
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                agent.action.u = np.zeros(self.world.dim_p)
                action_tmp = self.dec_to_base(action[0], 10)
                action_p = action_tmp[1]
                action_u = action_tmp[0]


                # process discrete action
                if action_u == 1: agent.action.u[0] = -1.0
                if action_u == 2: agent.action.u[0] = +1.0
                if action_u == 3: agent.action.u[1] = -1.0
                if action_u == 4: agent.action.u[1] = +1.0

                agent.action.p = (action_p+1)*0.1
    ...
    def render(self, mode='human'):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from multiagent_com import rendering
